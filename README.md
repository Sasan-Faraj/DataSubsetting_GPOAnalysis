...
To make things easier as well, feel free to download this [R script](filestructuredownload/RReference_GPOAnalysis.R "download") and this [excel](filestructuredownload/datafile_2_27 23.xlsx "download") file. **Prior to running the R script, ensure that both files are in the same folder.** The goal is that you can just change the file name for whatever xlsx file you want to use. You would potentially need to update the file structure as well. 
